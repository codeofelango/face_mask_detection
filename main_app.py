...
if __name__ == "__main__":
    # Loading OpenCV DNN for face detection
    ...
    maskNet = load_model("mobileNet_model_v2_4_1.h5")
    # initialize the video stream
    print("[INFO] starting video stream...")
    vs = cv2.VideoCapture(0)
    vs.set(3, 640)  # for width
    vs.set(4, 480)  # for height
    vs.set(10, 200)  # for brightness
    ...
    while True:
        ...
        for (box, pred) in zip(locs, preds):
            ...
            # Text and color to be displayed
            label = "Mask" if mask > withoutMask else "No Mask"
            color = (0, 255, 0) if label == "Mask" else (0, 0, 255)
            # include the probability in the Text label
            label = "{}: {:.2f}%".format(label, max(mask, withoutMask) * 100)

            # display bounding box rectangle & Text Label on the output  frame
